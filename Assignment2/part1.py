...
class Tree(object):
    ...
    def entropy(Y):
        ...
        e = -sum((count/total) * np.log2(count/total) for count in counter.values() if count!=0)
    ...
    def conditional_entropy(Y,X):
        ...
        a_counter = Counter(X)
        ce = 0.0
        for value in a_counter:
            counter = Counter([t for t,a in zip(Y,X) if a==value])
            a_entropy = Tree.entropy(counter)
            ce += (a_counter[value]/len(X)) * a_entropy
    ...
    def best_attribute(X,Y):
        ...
        for attribute in range(X.shape[0]):
            info_gain = Tree.information_gain(Y,X[attribute,:])
    ...
    def split(X,Y,i):
        ...
        for value in values:
            ...
            X_subset = X[:,indices]
            Y_subset = Y[indices]
            C[value] = Node(X_subset, Y_subset)
        #########################################
        return C
    ...
    def build_tree(t):
        ...
        best_index = Tree.best_attribute(t.X,t.Y)
        t.i = best_index

        t.p = Tree.most_common(t.Y)

        C = Tree.split(t.X,t.Y,best_index)
        t.C = {}
        for key, child_node in C.items():
            t.C[key] = Tree.build_tree(child_node)
    ...
    def inference(t,x):
        ...
        attribute = x[t.i]
        if attribute in t.C:
            return Tree.inference(t.C[attribute],x)
        return t.p
        #########################################
        #return y
    ...
    #--------------------------
    @staticmethod
    def load_dataset(filename = r'D:\MachineLearning\Assignment2\data\data1.csv'):
        '''
        ...
        data = np.genfromtxt(filename,delimiter=',',dtype=None,encoding=None).T
        X = np.array(data[1:, 1:])
        Y = np.array(data[0, 1:])
